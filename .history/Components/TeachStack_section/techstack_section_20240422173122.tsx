    ...
    <div>
      ...
      <div className="grid grid-cols-2 px-5 sm:grid-cols-3 max-w-3xl mx-auto aspect-square justify-center items-center gap-5 md:gap-x-7 my-3 mt-14 mb-16">
        {uniqueTechStack.map((tech, index) => (
          <div
            key={index}
            className="bg-white-50 border  w-25 h-auto flex flex-col items-center justify-center p-5 rounded-lg"
          >
            <div className="bg-white rounded-full p-3 border border-secondary-color overflow-hidden drop-shadow">
              {" "}
              {getIcon(tech, "w-11 h-11 p-1")}
            </div>
