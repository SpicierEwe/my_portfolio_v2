    ...
    if (Array.isArray(portfolioItem)) {
      portfolioItem.forEach((item) => {
        ...
        if (key.toLowerCase() === "ux") {
          console.log(item["case_study"]["mockups"]["phone_image_link"]);

          const mockups = item["case_study"]["mockups"];
          if (mockups["phone_image_link"]) {
            mockups["phone_image_link"].forEach((iamgeLink: string) => {
              links.push(iamgeLink);
            });
          }

          if (mockups["web_image_link"]) {
            mockups["web_image_link"].forEach((iamgeLink: string) => {
              links.push(iamgeLink);
            });
          }
        }
