          ...
          {(db.portfolioItems as any)[navItems[selectedNavItemIndex]].map(
            (item: any, index: number) => {
              ...
              return (
                <div
                  key={index}
                  className="cursor-pointer shadow-sm rounded-lg overflow-hidden mb-3 border border-gray-100 hover:child-scale-up"
                >
                  <Image
                    className="transition-transform duration-300 ease-in-out child-scale-up"
                    src={imageLink}
                    alt={projectName}
                    height={200}
                    width={200}
