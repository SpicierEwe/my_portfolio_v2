        ...
        <nav className="flex justify-center items-center p-5">
          ...
          <ul className="flex justify-center md:px-6">
            {navItems.map((item, index) => (
              ...
              <Link key={index} href="#">
                <li
                  className="cursor-pointer hover:bg-brand-color py-2 px-4 text-setTimeout(() => {
                  
                }, timeout); rounded-full hover:text-white  transition-colors duration-200 ease-in-out"
                >
                  {item.name}
                </li>
              </Link>
