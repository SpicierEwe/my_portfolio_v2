  ...
  return (
    <section>
      ...
      <div className="absolute h-screen w-screen bg-white z-30 p-5 pt-11">
        <div>
          <div className="absolute right-0 m-7 px-5 py-3.5 rounded-full bg-black text-white">
            <p>X</p>
          </div>
          ...
          <ul className="flex flex-col gap-5 mt-7">
            {navItems.map((item, index) => (
              <Link key={index} href={item.link}>
                <li
                  onClick={onMenuClick}
                  className="cursor-pointer hover:bg-brand-color py-2 rounded-full hover:text-white  transition-colors duration-200 ease-in-out"
                >
                  {item.name}
                </li>
              </Link>
