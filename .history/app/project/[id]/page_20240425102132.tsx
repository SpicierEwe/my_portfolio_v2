...
export default function Project({ params }) {
  ...
  const [projectData, setProjectData] = useState(getProjectData(id)[0]);

  useEffect(() => {
    if (projectData === undefined) setProjectData(getProjectData(id)[0]);
  }, [projectData, id]);
...
function getProjectData(id: string) {
  return Object.keys(db.portfolioItems).map((key) => {
    return (db.portfolioItems as any)[key].find((item: any) => {
      if (item.projectId === id) console.log(item);
